CREATE VIEW icredit.v_contacts_tech_spec (
    table_schema,
    table_name,
    column_name,
    id_role,
...
FROM (
    ...
    WHERE vft.table_name::text = 'contacts'::text OR vft.table_name::text =
        'contacts_pwd'::text
    ) f_sel
...
FROM icredit.v_dic_name_contacts_block vf,
    ...
    WHERE (v_field_type.table_name::text = 'contacts_adress'::text OR
        v_field_type.table_name::text = 'contacts_www'::text OR v_field_type.table_name::text = 'contacts_phones'::text OR v_field_type.table_name::text = 'contacts_documents'::text) AND v_field_type.table_schema::text = 'icredit'::text
    ) vft
WHERE ff.table_name::text = vft.table_name::text AND vf.value = ff.group_number
    AND ff.field_view_type::text = 'table'::text
ORDER BY 14, 15, 1, 2, 6, 3;