...
AS
  ...
  FROM (
         ...
         FROM icredit.contacts_block_access cba,
              ...
              icredit.contacts_page_number_access pna,
              icredit.dic_contacts_page_number dcpn
         ...
         WHERE cba.page_number = vncb.value AND
               vncb.parent_id = 0
         UNION
         SELECT vft.table_schema,
                ...
                CASE
                  ...
                  ELSE false::text
                END AS mandatory,
                ('v_'::text || ff.table_name::text) || '_tech_spec'::text AS
                  dic_name_r,
                CASE
         ...
         SELECT f_sel.table_schema,
                ...
                CASE
                  WHEN vdwr.dic_column_name IS NOT NULL AND vdwr_s.column_name
                    IS NULL THEN 'primary'::text
                  WHEN vdwr.dic_column_name IS NOT NULL AND vdwr_s.column_name
                    IS NOT NULL OR vdwr.dic_column_name IS NULL AND
                    vdwr_s.column_name IS NOT NULL THEN 'secondary'::text
                  ELSE 'none'::text
                END AS depends_type,
                ...
                CASE
                  ...
                  WHEN vdwr.dic_column_name IS NOT NULL AND vdwr_s.column_name
                    IS NOT NULL OR vdwr.dic_column_name IS NULL AND
                    vdwr_s.column_name IS NOT NULL THEN vdwr_s.column_name::
                    character varying
         ...
         FROM (
                SELECT vft.table_schema,
                       ...
                       vft.dic_name,
                       CASE
                         WHEN vft.dic_name::text ~~ 'dic%'::text THEN ('v_'::
                           text || vft.dic_name::text)::character varying
                       ...
                       COALESCE(ca.field_view_type, 'simple'::character varying)
                         AS field_view_type
                FROM icredit.contacts_block_access rgp,
                     icredit.contacts_page_number_access pna,
                     icredit.dic_contacts_page_number dcpn,
                     icredit.dic_table_for_v_contact_tech_sp dtf_ct,
                     icredit.v_field_type vft
                     LEFT JOIN icredit.dic_systems_fields ds ON vft.column_name
                       ::bpchar = ds.field_name::bpchar
                     LEFT JOIN icredit.contacts_access ca ON vft.column_name::
                       bpchar = ca.field_name::bpchar AND vft.table_name::bpchar
                       = ca.table_name::bpchar
                WHERE vft.table_name::text = dtf_ct.name::text AND
                      ca.group_number = rgp.group_number AND
                      ca.page_number = pna.page_number AND
                      pna.page_number = dcpn.value AND
                      dcpn.parent_id = 0
              ) f_sel
              LEFT JOIN icredit.contacts_caption cc ON f_sel.column_name::bpchar
                = cc.field_name::bpchar AND f_sel.table_name::bpchar =
                cc.table_name::bpchar
              ...
              (
                ...
                f_sel.dic_name::text = vdwr.dic_name::text AND
                f_sel.dic_column_name::text = vdwr.dic_column_name::text
         WHERE NOT (EXISTS (
